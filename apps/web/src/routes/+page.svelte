...
<style>
  ...
  /* Форма градиента сменена с ellipse (значение по умолчанию для
     ...
     угловых артефактов. Размер круга задан как farthest-corner — это
     единственный валидный синтаксис размера для circle в CSS (проценты,
     в отличие от ellipse, для circle не поддерживаются, поэтому
     "circle 130%" был невалиден и браузер просто отбрасывал background
     целиком — из-за этого свечение пропало на предыдущем скриншоте).
     Альфа по-прежнему в стопах (rgba), не в opacity — см. предыдущий
     комментарий про двойное квантование. */
  .hero-bg {
    position: absolute;
    inset: 0;
    pointer-events: none;
