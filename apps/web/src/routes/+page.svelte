...
<style>
  ...
  /* Один слой: тот же градиент (форма ellipse 125% 125%, мелкие rgba-стопы
     для устранения двойного квантования альфы), плюс лёгкий blur прямо на
     нём. Раньше blur на элементе с inset:0 давал заметную тень/кайму по
     скруглённым углам .hero-card — потому что размытая кромка градиента
     выходила ровно на границу overflow:hidden. Здесь слой отступает
     внутрь на 40px (inset: 40px вместо 0), так что после размытия его
     края остаются внутри контейнера с запасом и не создают видимую тень
     на скруглении. */
  .hero-bg {
    position: absolute;
    inset: 40px;
