...
<style>
  ...
  /* Альфа задана прямо в каждом стопе градиента (rgba), а не через
     element-level opacity. Если alpha применяется отдельным свойством
     opacity поверх уже растеризованного 8-битного градиента, браузер
     фактически квантует цвет дважды подряд — сначала при построении
     самого градиента, потом при наложении прозрачности — и именно это
     второе квантование давало заметные диагональные полосы в области
     низкой альфы (от transparent к первому цвету). Прописывая alpha в
     стопах, quantization происходит один раз, что убирает эффект. Много
     мелких стопов (шаг ~3%) дополнительно сглаживают переход. Итоговый
     видимый цвет на каждой позиции идентичен прежнему (был #f43f5e при
     opacity 0.28 = rgba(244,63,94,0.28) и т.д.). */
  .hero-bg {
    position: absolute;
    inset: 0;
    pointer-events: none;
