	...
	function gsapTransition(
		...
		if (direction === "in") {
			if (isFirst) {
				...
				return { duration: 0, tick: function _tick() { /* empty */ } };
			}
			...
			return { duration: 900, tick: function _tick() { /* empty */ } };
		}

		if (parent) parent.style.width = `${String(parent.offsetWidth)}px`;
		...
		return { duration: 300, tick: function _tick() { /* empty */ } };
	}
