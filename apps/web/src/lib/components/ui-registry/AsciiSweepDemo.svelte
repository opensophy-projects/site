<script lang="ts">
  ...
  function animate() {
    ...
    if (scanProgress >= 1) {
      ...
      panel1Opacity = currentPanel === 0 ? 1 : 0;
      panel2Opacity = currentPanel === 1 ? 1 : 0;
      return;
    }
    
    // Плавное изменение прозрачности панелей во время сканирования
    if (scanProgress > 0.3 && scanProgress < 0.7) {
      const fade = Math.sin((scanProgress - 0.3) / 0.4 * Math.PI);
      if (currentPanel === 0) {
        panel1Opacity = 1 - fade;
        panel2Opacity = fade;
      } else {
        panel2Opacity = 1 - fade;
        panel1Opacity = fade;
      }
    }
...
<div class="w-full max-w-5xl mx-auto px-4 py-8">
  ...
  <div bind:this={containerRef} class="relative w-full h-[500px] bg-background rounded-lg overflow-hidden border border-border">
    <!-- Панель 1 -->
    <div class="absolute inset-0 w-full h-full p-6 transition-opacity duration-300" style="z-index: 1; opacity: {panel1Opacity};">
      <FeatureCards cards={cards1} />
    </div>
    
    <!-- Панель 2 -->
    <div class="absolute inset-0 w-full h-full p-6 transition-opacity duration-300" style="z-index: 2; opacity: {panel2Opacity};">
      <FeatureCards cards={cards2} />
    </div>
    
    <!-- Canvas для ASCII эффекта -->
    <canvas 
      bind:this={canvasRef} 
  ...
  <div class="mt-6 grid gap-4 text-sm text-muted-foreground">
    <div class="bg-muted p-4 rounded-md">
      ...
      <ul class="list-disc list-inside space-y-1">
        <li><strong>band:</strong> 0.2 — ширина полосы сканирования</li>
