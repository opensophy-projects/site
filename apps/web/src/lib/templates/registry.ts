...
const rawTemplates = import.meta.glob<string>(
  "../../../../templates/**/*.svx",
  {
    query: "?raw",
    eager: true,
...
export function getTemplateEntries(): TemplateEntry[] {
  return Object.entries(rawTemplates)
    .map(([path, source]) => {
      ...
      const slug =
        typeof slugValue === "string" ? slugValue : slugFromTemplatePath(path);
    ...
    .filter((template): template is TemplateEntry =>
...
export function getTemplateEntry(slug: string) {
  return (
    getTemplateEntries().find((template) => template.slug === slug) ?? null
  );
}
